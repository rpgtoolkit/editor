 ...
 * deals with initializing most of the members needed by a concrete class, it also provides some of
 * the core functionality such as layer management. It does NOT perform the actual rendering of the
 * board it only defines abstract methods that a sub class can use, this is due to the fact that the
 * Toolkit supports both flat 2D boards and isometric boards.
 *
...
public abstract class AbstractBoardView extends JPanel implements
   ...
   *
   * @param index The position in the layers ArrayList to insert the new layer.
  ...
  /**
   * A concrete BoardView will implement its own program drawing code here.
   *
   * @param g The graphics context to draw on.
   */
  ...
  /**
   * Initializes a BoardView, it sets most of the BoardView's members but it does not do anything
   * with regard to the board model or parent editor.
   */
  private void init() {
