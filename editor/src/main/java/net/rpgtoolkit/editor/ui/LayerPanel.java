...
public class LayerPanel extends JPanel implements ChangeListener,
    ...
    if (e.getSource().equals(opacitySlider)) {
      if (boardView != null) {
        if (layerTable.getSelectedRow() > -1
                && layerTable.getRowCount() > 0) {
          boardView.getLayer((boardView.getBoard().getLayers().size()
  ...
  /**
   * TODO: It is possible that in the future other parts of the editor will be interested in layer
   * selection changes.
   *
   * Handles selection changes on the Layer Table, updating the opacity slider with the selected
   * layers current opacity.
   *
   * @param e
   */
  @Override
  public void valueChanged(ListSelectionEvent e) {
    ...
    if (boardView != null) {
      if (layerTable.getSelectedRow() > -1) {
        BoardLayerView selectedLayer = boardView.getLayer(
